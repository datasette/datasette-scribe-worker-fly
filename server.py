...
HUGGINGFACE_HUB_CACHE = environ["HUGGINGFACE_HUB_CACHE"]
PYANNOTE_CACHE = environ["PYANNOTE_CACHE"]
DB_PATH = environ["DB_PATH"]
HF_TOKEN = environ["HF_TOKEN"]
SCALE_TO_ZERO_DURATION = int(environ.get("SCALE_TO_ZERO_DURATION", "600"))

Path(MP3_DIRECTORY).mkdir(parents=True, exists_ok=True)
Path(PYANNOTE_CACHE).mkdir(parents=True, exists_ok=True)
Path(HUGGINGFACE_HUB_CACHE).mkdir(parents=True, exists_ok=True)
...
print("insanely-fast-whisper load time: ", time.time() - t0)
...
print("pyannote load time: ", time.time() - t0)
...
async def scale_to_zero_task():
    while True:
        ...
        duration = time.time() - last_request
    ...
    while True:
        ...
        while True:
            ...
            try:
                ...
                inputs, diarizer_inputs = preprocess_inputs(inputs=input_path)
                segments = diarize_audio(diarizer_inputs, diarization_pipeline)

                segmented = post_process_segments_and_transcripts(
                    segments, transcript1["chunks"], group_by_speaker=False
                )
                result = build_result(segmented, transcript1)

                queue.complete_whisper(job_id, result)
...
def job_status(job_id: str):
    ...
    if result["status"] == "failed":
        return {"error": True, "message": result["meta"]}
    if result["status"] == "completed":
        return {
            "completed": True,
            "transcript": json.loads(result["transcript"]).get("speakers"),
        }
    else:
